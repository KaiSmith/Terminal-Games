...
def LayTiles(rack,word,r,c,o,player):
    ...
    for l in range(len(word)):
        ...
        if Board[r][c] == '#':
            multiplier*=3
        if Board[r][c] == '2':
            score+=Points[word[l]]*2
        elif Board[r][c] == '3':
            score+=Points[word[l]]*3
        else:
            score+=Points[word[l]]
                ...
                if l == 0:
                    if r > 1:
                        if Board[r-1][c] in Letters:
                            score+=CountChain(r,c,'u',False)
                if l == len(word)-1:
                    if r < 15:
                        if Board[r+1][c] in Letters:
                            score+=CountChain(r,c,'d',False)
                if c > 0:
                    if Board[r][c-1] in Letters:
                        score+=CountChain(r,c,'l',True)
                if c < 14:
                    if Board[r][c+1] in Letters:
                        score+=CountChain(r,c,'r',True)
            else:
                if l == 0:
                    if c > 0:
                        if Board[r][c-1] in Letters:
                            score+=CountChain(r,c,'l',False)
                if l == len(word)-1:
                    if c < 14:
                        if Board[r][c+1] in Letters:
                            score+=CountChain(r,c,'r',False)
                if r > 1:
                    if Board[r-1][c] in Letters:
                        score+=CountChain(r,c,'u',True)
                if r < 15:
                    if Board[r+1][c] in Letters:
                        score+=CountChain(r,c,'d',True)
